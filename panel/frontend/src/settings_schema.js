...
// 每项字段:
//   label  中文名
//   desc   中文说明 (基于官方英文说明翻译)
//   type   bool | int | float | text | select   —— 决定前端用什么控件
//   options  仅 select 用, 下拉可选值
//   managed  true 表示该项由 game/server.env 在每次启动时重新写入,
//            面板里改了重启会被覆盖, 故设为只读并给出提示
//   note   额外补充 (性能/范围提示等)
//
// 注: RCON 在本项目已废弃, 面板与后台全程使用官方 REST API, 故不含 RCONEnabled/RCONPort。
// ============================================================================
...
export function toIniValue(type, v) {
  if (type === 'bool') return v ? 'True' : 'False'
  return String(v)
}

// 从 ini 原始字符串解析为控件用值
export function fromIniValue(type, raw) {
